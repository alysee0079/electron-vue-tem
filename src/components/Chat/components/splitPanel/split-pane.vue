...
<template>
  <div :class="wraperClasses" ref="wraper" @mouseup="handleMouseup" @mousemove="handleMousemove" @mouseleave="handleMouseout">
    <div v-if="direction === 'horizontal'" :class="`${prefix}-container`">
      <div :class="`${prefix}-left-area`" :style="{ width: leftSize }">
        <slot name="left"></slot>
      </div>
      <slot name="trigger">
        <div :class="`${prefix}-trigger`" ref="trigger" :style="horizontalTriggerStyle" @mousedown="handleMousedown" unselectable="on">
          <div class="trigger-middle-point">
            <p></p>
            <p></p>
            <p></p>
            <p></p>
    ...
    <div v-else :class="`${prefix}-container`">
      ...
      <slot name="trigger">
        <div :class="`${prefix}-trigger`" ref="trigger" :style="verticalTriggerStyle" @mousedown="handleMousedown" unselectable="on">
          <div class="trigger-middle-point">
            <p></p>
            <p></p>
            <p></p>
...
export default {
  ...
  mounted() {
    if (this.value !== undefined) {
      this.$nextTick(() => {
        this.triggerOffset =
          typeof this.value === 'number' ? this.value : Math.floor((parseInt(this.value) / this.$refs.wraper.offsetWidth) * 10000) / 100
      })
      this.triggerOffset =
        typeof this.value === 'number' ? this.value : Math.floor((parseInt(this.value) / this.$refs.wraper.offsetWidth) * 10000) / 100
    }
